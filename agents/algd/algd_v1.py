...
class ALGDAgent(Agent):
    ...
    def update_actor(self, state, action_taken):
        ...
        if self.args.qc_ens_size == 1:
            actor_std = torch.zeros_like(actor_mean).to(self.device)
        ...
        actor_loss = torch.mean(
            - actor_Q + (lam - self.rect) * actor_QC
        )
...
#         if updates % 100 == 0:  # 不要每步都 print，会很吵
#             print(
#                 f"[time] prep={t1-t0:.4f}s, critic={t2-t1:.4f}s, actor={t3-t2:.4f}s"
#             )
