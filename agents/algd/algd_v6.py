...
from agents.algd.model import QNetwork, DiffusionPolicy, QcEnsemble
...
class ALGDAgent(Agent):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.T = getattr(args, "diffusion_T", 5) # 5
        ...
        self.score_sigma_scale = getattr(args, "score_sigma_scale", 1.0) #1.0
        # softmax 温度 β：w_i ∝ exp( -L_A / β )
        self.score_beta = getattr(args, "score_beta", 1.0) # 1.0
        ...
        self._mc_time_M2 = 0.0   # sum of square diffs
    ...
    def compute_L(self, state, action):
        ...
        qc_std, qc_mean = torch.std_mean(QCs, dim=0)    # [B,1]
    ...
    def compute_score_target_mc(self, state, a_tau, tau):
        ...
        sigma_all = self.policy.ve_sigmas           # [T]
        sigma_tau = self.score_sigma_scale * sigma_all[tau]  # [B]
        sigma_tau = sigma_tau.view(B, 1, 1)        # 方便广播到 [B, N, act_dim]
    ...
    def update_actor(self, state, action_taken):
        ...
        with torch.no_grad():
            self.last_log = {
                ...
                "score_mc_time_ms_mean": float(self.get_score_mc_time_stats()["mean_ms"]),
                "score_mc_time_ms_std": float(self.get_score_mc_time_stats()["std_ms"]),
                "score_mc_time_n": int(self.get_score_mc_time_stats()["n"]),
            }
